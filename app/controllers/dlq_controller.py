...
async def get_dlq_stats(
    ...
    try:
        ...
        dlq = DLQManager(
            ...
            dlq_queue_name="governance-events.dlq"
        )
        
        async with dlq:
            stats = await dlq.get_statistics()
        
        tracer.finish_span(trace_parent, "api.get_dlq_stats", True)
        
        return {"success": True, "data": stats, "trace_id": trace_id}
...
async def peek_dlq_messages(
    ...
    try:
        ...
        tracer.finish_span(trace_parent, "api.peek_dlq_messages", True)
        
        return {"success": True, "messages": messages, "count": len(messages), "trace_id": trace_id}
...
async def replay_dlq_message(
    ...
    try:
        ...
        tracer.finish_span(trace_parent, "api.replay_dlq_message", True)
        
        return {"success": success, "event_id": event_id, "trace_id": trace_id}
...
async def replay_dlq_batch(
    ...
    try:
        ...
        tracer.finish_span(trace_parent, "api.replay_dlq_batch", True)
        
        return {"success": True, "data": result, "trace_id": trace_id}
...
async def purge_dlq_messages(
    ...
    try:
        ...
        tracer.finish_span(trace_parent, "api.purge_dlq_messages", True)
        
        return {"success": True, "purged_count": purged, "age_hours": age_hours, "trace_id": trace_id}
